...
The consequence is explicit: an issued token cannot be invalidated before `exp`. There is no blacklist, revocation store, account-status lookup, or refresh-token mechanism in v0.0.1. Expiry is the only containment mechanism.
