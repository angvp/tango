...
- **No framework-installed OS signal handling.** `tango.ServeContext` shuts down gracefully when its `ctx` is canceled, but never installs a `SIGTERM`/`SIGINT` handler itself — a host wires `signal.NotifyContext` in its own `main`, as `examples/realtime-chat` does. `Hub.Close` (and any other component) is wired in via `Registry.RegisterLifecycle`, not a bespoke shutdown path — see [application lifecycle](guides/application-lifecycle.md).
...
- **No lifecycle status in `-tango-status`.** The `-tango-status` JSON payload reports registration/database/migration state only; it says nothing about whether any registered `Lifecycle` component is currently running.
- **Graceful shutdown may take up to roughly 2x `WithShutdownTimeout`.** HTTP draining and `Lifecycle.Stop` calls each get their own independent timeout budget, not one shared deadline — see [ADR 0031](adr/0031-shutdown-uses-two-independent-phase-timeouts-and-a-decoupled-application-context.md). Account for this when tuning a container's overall termination grace period.
