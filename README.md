...
- A model metadata engine that reads plain Go structs and a handful of tags (`tango:"pk"`, `tango:"unique"`, `tango:"index"`).
- A minimal `db.Store` for CRUD and raw SQL against SQLite or PostgreSQL.
- Generated, typed Go migrations (`tango makemigrations`/`migrate`/`migrate down`) — no separate schema DSL.
- A server-rendered HTML admin (list/create/edit/delete, pagination, sorting, search) behind Basic Auth.
- A CLI (`tango run`/`check`/`makemigrations`/`migrate`/`newproject`/`newapp`/`tui`) that wraps ordinary `go build`/`go run` rather than replacing them.
...
`tango newproject` scaffolds a `go.mod` and a `main.go` pre-wired for SQLite, with no app registered yet. `tango newapp` scaffolds an app stub; wiring it into `main.go`'s `InstalledApps` is one line you write yourself — tanGO never edits your `main.go` for you. The example's app registers one named route:
...
- **[Guides](docs/guides/)** — standalone, task-oriented references: project structure, configuration, models and tags, routing, `Context`, persistence, migrations, admin, app checks, and dialect setup.
...
- **[Examples](examples/)** — `jsonapi` (JSON-only) and `api-with-admin` (JSON API and HTML admin sharing the same models).
