...
// -tango-admin-deactivate=<username>) and, if one is present, performs the
// corresponding account operation against store and reports handled=true.
// If none is present it returns handled=false, nil so the caller (a
// generated main.go) proceeds to its other flag handling / serving.
//
...
func HandleCLI(ctx context.Context, store *db.Store, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) (bool, error) {
	if username, ok := flagValue(args, "-tango-admin-create"); ok {
		...
		if err := CreateAccount(ctx, store, username, password); err != nil {
			return true, err
		}
